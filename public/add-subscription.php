...
<body class="bg-background-light font-sans text-gray-900 antialiased overflow-hidden h-screen flex transition-colors duration-300">
    ...
    <main class="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header class="flex items-center justify-between px-8 py-6 z-10 gap-8 shrink-0">
            <div class="flex items-center gap-8 flex-1">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">新增订阅</h1>
                    <p class="text-text-secondary-light mt-1 text-sm">添加新的订阅服务以追踪支出</p>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto z-10 px-8 pb-8">
            <form action="api/subscriptions/add.php" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto space-y-6">
                    ...
                    <div class="flex items-center gap-5 mb-6">
                        <div class="shrink-0 group relative self-center">
                            <div class="w-16 h-16 rounded-full bg-gray-50 border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 hover:border-brand-blue hover:text-brand-blue transition-all cursor-pointer overflow-hidden" onclick="document.getElementById('logo-upload').click()">
                                ...
                                <span id="logo-placeholder" class="material-symbols-outlined text-3xl">add_photo_alternate</span>
                            </div>
                            <input type="file" id="logo-upload" name="logo_file" accept="image/*" class="hidden" onchange="handleLogoUpload(this)"/>
                            <input type="hidden" name="logo_url" id="logo-url" value="">
                        </div>
                        <div class="flex-1 space-y-4">
                            <div>
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider pl-1 block mb-1.5">服务名称</label>
                                <div class="relative group">
                                    <input class="input-transition block w-full rounded-2xl border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400 focus:bg-white focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 py-2.5 pl-10 sm:text-sm font-medium" name="name" placeholder="例如: Netflix" type="text" required/>
                            ...
                            <div class="w-full sm:w-[40%]">
                                ...
                                <div class="relative group">
                                    <input class="input-transition block w-full rounded-2xl border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400 focus:bg-white focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 py-2.5 pl-10 sm:text-sm" name="logo_url_remote" placeholder="https://..." type="url"/>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center gap-4 w-full">
                            <div class="relative flex-1 min-w-0">
                                ...
                                <input class="input-transition block w-full rounded-2xl border-gray-200 bg-white text-gray-900 py-2.5 pl-10 pr-4 focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 sm:text-sm font-semibold text-center" name="interval_value" value="1" min="1" max="366" type="number"/>
                            </div>
                            <div class="relative flex-1 min-w-0">
                            ...
                            <div class="flex items-center h-[42px] px-3 rounded-xl shrink-0">
                                <input class="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary bg-gray-50 cursor-pointer" id="is_lifetime" name="is_lifetime" type="checkbox"/>
                    ...
                    <div class="w-full flex items-start gap-4 flex-wrap">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="relative rounded-2xl shadow-subtle bg-white group focus-within:ring-[3px] focus-within:ring-brand-blue/15 transition-all">
                                ...
                                <input class="input-transition block w-full rounded-2xl border-gray-200 bg-transparent pl-8 pr-28 py-2.5 text-gray-900 placeholder-gray-300 focus:border-brand-blue focus:ring-0 sm:text-sm font-bold tracking-tight shadow-none z-10 relative" id="amount" name="amount" placeholder="0.00" type="text"/>
                                    ...
                                    <div class="relative h-full">
                                        <select class="h-full rounded-r-2xl border-0 bg-transparent py-0 pl-2 pr-8 text-gray-500 font-medium focus:ring-0 sm:text-xs text-right cursor-pointer appearance-none hover:text-gray-700 transition-colors" id="currency" name="currency_id">
                                            <?php foreach ($currencies as $curr): ?>
                                                <option value="<?php echo $curr['id']; ?>" <?php echo $curr['code'] === $defaultCurrency ? 'selected' : ''; ?>>
                            ...
                            <div class="relative">
                                <select class="input-transition block w-full rounded-2xl border-gray-200 bg-white text-gray-900 py-2.5 pl-10 pr-8 focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 sm:text-sm font-medium appearance-none cursor-pointer truncate" name="payment_method_id">
                                    ...
                                    <?php foreach ($paymentMethods as $pm): ?>
                                        <option value="<?php echo $pm['id']; ?>" <?php echo $pm['code'] === $defaultPaymentMethod ? 'selected' : ''; ?>>
                            ...
                            <div class="relative">
                                <select class="input-transition block w-full rounded-2xl border-gray-200 bg-white text-gray-900 py-2.5 pl-10 pr-8 focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 sm:text-sm font-medium appearance-none cursor-pointer truncate" name="category_id">
                                    ...
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?php echo $cat['id']; ?>" <?php echo $cat['code'] === $defaultCategory ? 'selected' : ''; ?>>
                ...
                <div class="bg-gray-50 rounded-3xl p-6 border border-gray-200 space-y-6">
                    <div class="flex items-center justify-between">
                        ...
                        <button id="auto_renewal_toggle" type="button" class="bg-brand-blue relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-blue focus:ring-offset-2" role="switch" aria-checked="true">
                            <span id="auto_renewal_knob" aria-hidden="true" class="translate-x-5 pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"></span>
                        </button>
                        <input type="hidden" name="auto_renew" id="auto_renew_input" value="1"/>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 pl-1">开始日期</label>
                            <div class="relative">
                                <input class="input-transition block w-full rounded-2xl border-gray-200 bg-white text-gray-900 shadow-subtle focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 sm:text-sm py-2.5 px-3" name="start_date" type="date" required/>
                            </div>
                        </div>
                        <div>
                ...
                <div class="bg-card-light rounded-3xl p-8 shadow-soft space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex gap-2">
                                ...
                                <div class="relative flex-1">
                                    <input class="input-transition block w-full rounded-2xl border-gray-200 bg-white text-gray-900 shadow-subtle focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 sm:text-sm py-2.5 px-1 text-center cursor-pointer" name="remind_time" type="time" value="09:00"/>
                                </div>
                            </div>
                        </div>
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                <input class="input-transition block w-full rounded-2xl border-gray-200 bg-white text-gray-900 placeholder-gray-400 shadow-subtle focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 sm:text-sm py-2.5 pl-10" name="website_url" placeholder="https://" type="url"/>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea class="input-transition block w-full rounded-2xl border-gray-200 bg-white text-gray-900 placeholder-gray-400 shadow-subtle focus:border-brand-blue focus:ring-[3px] focus:ring-brand-blue/15 sm:text-sm py-3 px-4 resize-none" name="note" placeholder="添加账号信息、备忘录等..." rows="3"></textarea>
                    </div>
                </div>
    ...
    <script>
        ...
        let isAutoRenewalOn = true;

        autoRenewalToggle.addEventListener('click', function() {
            isAutoRenewalOn = !isAutoRenewalOn;
            autoRenewalToggle.setAttribute('aria-checked', isAutoRenewalOn);
            autoRenewalInput.value = isAutoRenewalOn ? '1' : '0';
        ...
        function calculateNextPaymentDate() {
            ...
            // Update the disabled next payment date field
